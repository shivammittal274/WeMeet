...
import {MutedMic, UnmutedMic, MutedVideo, UnmutedVideo, Screen, Unscreen, 
	Msg, SendMsg, EndCall, CCbtn, UpArrow, WhatsappBtn, MailBtn, RecBtn, EventNoteIcon, WhiteBoardIcon,EmojiIcon} from '../../scripts/buttons';
...
import Time from '../Home/Header/Time';
...
import "./Video.css";
import filter from './BadWordBlockage';
...
const server_url = "/" || "https://wemeetgrouptalk.herokuapp.com" || "http://localhost:4000"
...
const peerConnectionConfig = {
  iceServers: [
    ...
    {
      urls: "turn:turn.pranavms.ml:3478",
      username: "pranavms",
      credential: "pranavms@13",
    },
    {
      urls: "turn:turn.pranavms.ml:5349",
      username: "pranavms",
      credential: "pranavms@13",
    },
...
class Video extends Component {
  ...
  componentDidMount() {
    if (!cookie.load("EVAC") || !verifytok(cookie.load("EVAC"))) {
      ...
      if (
        ...
        window.location.pathname.split("/")[2].length === 14
      ) {
      ...
      this.setState(
        {
          username: cookie.load("EVAC").email,
          fullName: cookie.load("EVAC").name,
        },
        () => {
          // this.getPermissions()
          this.getMedia();
  ...
  generateNameStream = (name) => {
    ...
    canvas.width = 640;
    canvas.height = 480;
    canvas.style.backgroundColor = "#041629";
    canvas.borderRadius = "50%";
    var c = canvas.getContext("2d");
    // c.fillStyle = '#ffffff';

    var gradient = c.createLinearGradient(0, 0, canvas.width, 0);
    gradient.addColorStop("0", "#041629");
    gradient.addColorStop("1", "#041629");
    // Fill with gradient
  ...
  getMedia = async () => {
    ...
    if (!this.state.video && !this.state.audio && !this.state.screen) {
      ...
      navigator.mediaDevices
        .getDisplayMedia({ video: this.state.screen, audio: this.state.screen })
        .then(this.getMediaSuccess)
        .then((stream) => {})
        .catch((e) => {
          console.log(e);
      ...
      navigator.mediaDevices
        ...
        .then((stream) => {
          stream.addTrack(
            this.generateNameStream(this.state.fullName).getVideoTracks()[0]
          );
        })
        .then(this.getMediaSuccess)
        .catch((e) => console.log(e));
        ...
        clients.forEach((client) => {
          ...
          if (connections[client] === undefined) {
            ...
            connections[client].onaddstream = (event) => {
              let clientvideo = document.querySelector(
                `[data-socket="${client}"]`
              );
              if (clientvideo !== null) {
  ...
  handleVideo = () => {
    ...
    this.state.video
      ? message.error("Video is Off")
      : message.success("Video is On");
  };
  ...
  handleAudio = () => {
    ...
    this.state.audio
      ? message.error("Microphone is muted")
      : message.success("Microphone is unmuted");
  };
  ...
  handleScreen = () => {
    ...
    this.state.screen
      ? message.error("Screen Sharing is OFF")
      : message.success("Screen Sharing is ON");
  };
  ...
  downLoadAsPdf = () => {
    ...
    pdfDoc.text(this.state.notes, 10, 10);
  ...
  handlecc = () => {
    ...
    if ("webkitSpeechRecognition" in window) {
      ...
      message.error(
        "Closed Captions not Supported on " + window.navigator.appName
      );
    }
  };
  ...
  submitFile=()=>{
    ...
    const uploadTask = storage.ref(`files/${this.state.file.name}`).put(this.state.file);
    uploadTask.on(
      "state_changed",
      snapshot => {
  ...
  openChat = () => {
    this.setState({ showChat: true, newmessages: 0 });
  };
  openMeet = () => {
  ...
  addMessage = (data, sender) => {
    this.setState((prevState) => ({
      messages: [
        ...prevState.messages,
        { sender: data.sender, data: data.msg, msgType : data.msgType, fileName:data.fileName },
      ],
    }));

    if (sender !== socketId || !this.state.showChat) {
  ...
  sendMessage = () => {
    if (this.state.message === "") {
      ...
      var finalMsg = filter.clean(this.state.message);
      socket.emit("chat-message", {
        sender: this.state.fullName,
        msg: finalMsg,
        msgType: "text",
  ...
  openBoard=()=>{
    window.open(
      "/whiteBoard/" + window.location.pathname.split("/")[2], "_blank");
      ...
      >
        <div>
          <div>
            ...
            < div className = "btn-down timed" >
              <Time/>
            </div>
            <div
              className="btn-down bombat"
              style={{
                height: "70px",
                maxHeight: "70px",
                paddingTop: "3px",
                ...
                boxShadow: "10px 0 10px 20px #E5E7E9",

                borderRadius: "25px",
              }}
            >

              <UpArrow fontSize="1.8em" margin="5px" onClick={this.openMeet} />
              {this.state.video ? (
                <UnmutedVideo
                  margin="5px"
                  fontSize="1.8px"
              ...
              {this.state.audio ? (
                <UnmutedMic margin="5px" onClick={this.handleAudio} />
              ) : (
                <MutedMic margin="5px" onClick={this.handleAudio} />
              )}

              <span style={{ marginLeft: "3%", marginRight: "3%" }}>
                <EndCall margin="5px" onClick={this.handleEndCall} />
              </span>


              {this.state.screenAvailable && this.state.screen && (
                <Screen margin="5px" onClick={this.handleScreen} />
              )}
              {this.state.screenAvailable && !this.state.screen && (
                <Unscreen margin="5px" onClick={this.handleScreen} />
              )}
              ...
              {this.state.screenAvailable && (
                <RecBtn margin="5px" onClick={this.handleRec} />
              )}
              {/* {<button onClick={this.handleClick}> */}
              < WhiteBoardIcon margin="5px" onClick={this.openBoard}/>
                < EventNoteIcon margin="5px" onClick={this.handleClick}/>
        ...
        <div className='notes'>
          <div className='btn-div'>
            ...
            <div className='flex-btns-div'>
              <button type='primary' onClick={this.downLoadAsPdf}>
                <GetAppIcon />
              </button>
          ...
          />
            ...
            >
              <Modal.Header className="bombat" closeButton>
                <Modal.Title>WeMeet ChatRoom</Modal.Title>
              </Modal.Header>
              <Modal.Body
                style={{ overflow: "auto", overflowY: "auto", height: "400px" }}
              >
                {this.state.messages.length > 0 ? (
                  this.state.messages.map((item, index) => (
                    <div key={item.sender + item.data + index}>
                      <div className="msg">
                        {item.msgType =="text" ? (item.data):
                        ( <div className="downloadFile">
                          <p className="filename">{item.fileName}</p>
                          <a href={item.data} target="_blank" download ><GetAppIcon className="downloadIcon" /></a>
                          </div>)
              ...
              <Modal.Footer className="div-send-msg">
                ...
                < Picker className="colorPicker" pickerStyle={{position:"absolute",right:"350px",bottom:"45px"}} onEmojiClick = {
                  this.onEmojiClick
                  }
                  />
                ) : ("")
              }
                <input type="file" onChange={this.handleFile} id="icon-button-video" hidden/>
                < label htmlFor = "icon-button-video" >
                  <IconButton color="primary" component="span" style={{padding:"0px"}}>
                  < AttachFileIcon />
                  </IconButton>
                </label>
                
                
                {/* {this.state.file ?"Hi ":"No"} */}
                <PublishIcon type="submit" onClick={this.submitFile}/>
            
                
                < EmojiIcon onClick= {this.setEmoji}/>
                ...
                />
                  {this.state.uploading ? <small className="uploadDocText">Wait: Uploading</small> :""}
                {/*<Button variant="contained" color="primary" onClick={this.sendMessage}>Send</Button>*/}
              </Modal.Footer>
            </Modal>

            <Modal
              show={this.state.showMeet}
              onHide={this.closeMeet}
              style={{ zIndex: "999999" }}
            >
              <Modal.Header className="bombat" closeButton>
                <Modal.Title>WeMeet Meeting Details</Modal.Title>
              </Modal.Header>
              <Modal.Body
                style={{ overflow: "auto", overflowY: "auto", height: "220px" }}
              >
                ...
                >
                  <WhatsappBtn
                    margin="5px"
                    host={this.state.fullName}
                    ml={window.location.href}
                    mc={window.location.href.split("/")[4]}
                  />
                  <MailBtn
                    margin="5px"
                    host={this.state.fullName}
